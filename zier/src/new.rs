...
pub(crate) fn create_new_project(name: String, root: &Path, lib: bool) -> eyre::Result<()> {
    ...
    if lib {
        ...
        std::fs::write(lib_root, ZIER_LIB)?;
    } else {
        let bin_root = src_dir.join(BIN_ENTRY_POINT);
        std::fs::write(bin_root, ZIER_HELLO_WORLD)?;
    }

    Ok(())
}

const ZIER_HELLO_WORLD: &str = r#"(use std/io)

(let main {}
  (io/println "hello world~n"))
"#;

const ZIER_LIB: &str = r#"(pub extern let println ~ (String -> Unit) io/format)"#;
...
mod tests {
    ...
    fn unique_temp_root() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("zier-new-test-{}-{nanos}", std::process::id()))
    }
