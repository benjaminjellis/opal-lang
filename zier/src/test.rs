...
pub(crate) fn test(project_dir: &Path) -> eyre::Result<()> {
    ...
    for (module_name, source) in &test_module_sources {
        ...
        let test_fns = zierc::test_declarations(source);
    ...
    for (_, source) in &test_module_sources {
        for (_, mod_name, _) in zierc::used_modules(source) {
    ...
    let runner_path = build_dir.join("zier_test_runner.erl");
