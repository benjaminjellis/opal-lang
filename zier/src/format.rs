...
use crate::{MANIFEST_NAME, SOURCE_DIR, utils::find_zier_files};
...
pub(crate) fn format_fie(source_file_path: &PathBuf) -> eyre::Result<()> {
    ...
    let formatted_file = zier_format::format(&source_file, LINE_WIDTH);
...
pub(crate) fn format_dir(dir: &Path) -> eyre::Result<()> {
    let zier_files = find_zier_files(dir);

    for source_file_path in zier_files {
