...
struct Ctx {
    ...
    /// Top-level function name → number of Zier args (for multi-arg TCO)
    ...
    /// User-facing module name → Erlang module name (e.g. "io" → "zier_io")
    ...
    for decl in decls {
        match decl {
            ...
            ast::Declaration::Test { body, .. } => {
                functions.push(ir::Function {
                    name: format!("zier_test_{test_idx}"),
...
pub fn emit_module(module: &ir::Module) -> String {
    ...
    // Export all functions. Zier privacy is enforced by the import system, not Erlang exports.
