...
impl TypeChecker {
    ...
    pub fn infer(
        ...
        match expr {
            ...
            // Named function definition — always self-recursive in Zier.
        ...
        for decl in decls {
            match decl {
                ...
                Declaration::Test { name, body, span } => {
                    ...
                    let _ = (name, span); // used for discovery by zier, not the typechecker
                }
            }
        }
...
mod tests {
    ...
    fn unbound_variable_error() {
        ...
        let mut lowerer = crate::lower::Lowerer::new();
        let file_id = lowerer.add_file("test.zier".into(), "x".into());
        let sexprs = crate::sexpr::SExprParser::new(tokens, file_id)
            .parse()
            .unwrap();
    ...
    fn type_mismatch_error() {
        ...
        let mut lowerer = crate::lower::Lowerer::new();
        let file_id = lowerer.add_file("test.zier".into(), "(+ True 1)".into());
        let sexprs = crate::sexpr::SExprParser::new(tokens, file_id)
            .parse()
            .unwrap();
        let expr = lowerer.lower_expr(file_id, &sexprs[0]).unwrap();
